...
* **Root (`.`)**: The apex of the DNS hierarchy. Every fully qualified domain name (FQDN) ends with an implicit or explicit dot, indicating the root(append by system/browser).
...
* **Green** = DNS resolver-initiated steps(IPS Perform)
...
> Query always goes to root servers, and entire heirarchy work to resolve dns.
...
* Reduce load on DNS Authoritive server.
...
* All 13 root servers hosting the same zone file.
...
Root server zone only updated when there is new TLD.
