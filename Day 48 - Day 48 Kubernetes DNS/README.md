...
```bash
controlplane:~$ kubectl run nginx1 --image=nginx 
pod/nginx1 created

controlplane:~$ kubectl run nginx2 --image=nginx 
pod/nginx2 created

controlplane:~$ kubectl get pods -owide
NAME     READY   STATUS    RESTARTS   AGE   IP            NODE     NOMINATED NODE   READINESS GATES
nginx1   1/1     Running   0          15s   192.168.1.4   node01   <none>           <none>
nginx2   1/1     Running   0          11s   192.168.1.5   node01   <none>           <none>

controlplane:~$ kubectl get pods -owide -n kube-system
NAME                                      READY   STATUS    RESTARTS      AGE   IP            NODE           NOMINATED NODE   READINESS GATES
calico-kube-controllers-7bb4b4d4d-5x7wv   1/1     Running   2 (51m ago)   26d   192.168.0.2   controlplane   <none>           <none>
canal-hwn8t                               2/2     Running   2 (51m ago)   26d   172.30.2.2    node01         <none>           <none>
canal-kkbdg                               2/2     Running   2 (51m ago)   26d   172.30.1.2    controlplane   <none>           <none>
coredns-76bb9b6fb5-6hd7r                  1/1     Running   1 (51m ago)   26d   192.168.1.3   node01         <none>           <none>
coredns-76bb9b6fb5-zsjp6                  1/1     Running   1 (51m ago)   26d   192.168.1.2   node01         <none>           <none>
etcd-controlplane                         1/1     Running   3 (51m ago)   26d   172.30.1.2    controlplane   <none>           <none>
kube-apiserver-controlplane               1/1     Running   3 (51m ago)   26d   172.30.1.2    controlplane   <none>           <none>
kube-controller-manager-controlplane      1/1     Running   2 (51m ago)   26d   172.30.1.2    controlplane   <none>           <none>
kube-proxy-cf4m9                          1/1     Running   1 (51m ago)   26d   172.30.2.2    node01         <none>           <none>
kube-proxy-k5v4v                          1/1     Running   2 (51m ago)   26d   172.30.1.2    controlplane   <none>           <none>
kube-scheduler-controlplane               1/1     Running   2 (51m 

controlplane:~$ kubectl expose pod nginx1 --port=80 --target-port=80
service/nginx1 exposed
controlplane:~$ kubectl get svc
NAME         TYPE        CLUSTER-IP      EXTERNAL-IP   PORT(S)   AGE
kubernetes   ClusterIP   10.96.0.1       <none>        443/TCP   26d
nginx1       ClusterIP   10.101.152.94   <none>        80/TCP    6s

controlplane:~$ kubectl get ep
Warning: v1 Endpoints is deprecated in v1.33+; use discovery.k8s.io/v1 EndpointSlice
NAME         ENDPOINTS         AGE
kubernetes   172.30.1.2:6443   26d
nginx1       192.168.1.4:80    14s

controlplane:~$ nslookup nginx1.default.svc.cluster.local 192.168.1.3
Server:         192.168.1.3
Address:        192.168.1.3#53

Name:   nginx1.default.svc.cluster.local
Address: 10.101.152.94

controlplane:~$ nslookup nginx1.default.pod.cluster.local 192.168.1.3
Server:         192.168.1.3
Address:        192.168.1.3#53

** server can't find nginx1.default.pod.cluster.local: NXDOMAIN
...
│ • This command CUSTOMIZES the shell prompt                   │
│                                                              │
│ BREAKDOWN                                                     │
│ • \e[1;32m  → Green color (bold)                              │
│ • \u        → Current username                                │
│ • @         → Literal '@'                                     │
│ • \h        → Hostname (container/pod name)                   │
│ • :         → Separator                                       │
│ • \w        → Current working directory                        │
│ • $         → Shell symbol ($ for normal user)                │
│ • \e[0m     → Reset color                                     │
│                                                              │
│ RESULTING PROMPT                                             │
│ • Shows: root@debug-pod:~$                                   │
│ • In GREEN color                                              │
│                                                              │
│ PURPOSE                                                       │
│ • Better readability                                         │
│ • Easy identification of user, pod, and path                 │
│ • Commonly used in containers & servers                      │
│                                                              │
└──────────────────────────────────────────────────────────────┘
```
---

* By default kind clusyer enabled the pod dns entries.
* we can lookup the pod `nslookup <hyphenated-ip>.<namespace>.pod.cluster.local`
...
```bash
kubectl describe deploy kube-coredns -n kube-system 
