### References:-
- [Day 38: Admission Controllers in Kubernetes | Mutating & Validating](https://www.youtube.com/watch?v=iZTQv4F7Djk&ab_channel=CloudWithVarJosh)
...
* You can outsource the authentication or authorization thing but the final decision is take place by the API-Server.
...
| `AlwaysPullImages`         | Yes      | Yes        | Forces Pods to **always pull images** from the registry, preventing use of cached or potentially tampered images, enhancing container image security in multi-user environments. imagepullpolicy-Always and tag image with version to make sure image alwqys pulled even it is cached to prevent malicious pod to gain advantages of cached images| No                 |
...
They run inside of cluster but in extreamly rare scenario it can also be run outside of cluster.
...
Try To disable a `serviceaccount` admission controller and see no service account token mounted on pod.
...
Admission controllers only active when requiest is updating or creating any data(write) in etcd, GET request not get into the admission phase.
