### References:-
- [Day 39: Custom Resources (CR) and Custom Resource Definitions (CRD)](https://www.youtube.com/watch?v=y4e7nQzu_8E)
...
- https://www.tutorialspoint.com/kubernetes/kubernetes_custom_resource_definitions.htm

---

In kubernetes there are built-in objects that provided by kubernetes, but some time we need your own objects that fullfill our requirement we can create custom objects using CRD, CR and Custom Controllers(optional).
...
The `.status` field is used by controllers to **reflect the current state of the resource** — for example, the **last backup time**, whether the most recent backup **succeeded or failed**, or whether **old backups were purged**. While `.spec` declares *what should happen*, `.status` reveals *what actually happened* in the cluster.
