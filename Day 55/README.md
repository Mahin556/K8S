...
```bash
Sometimes CNI / CSI / CRI or other add-ons
depend on newer Kubernetes API features.

If your cluster is running an older Kubernetes version,
those new features may not exist.

Result:
- Add-on may fail to start
- Feature may not work
- You may see errors
- Version mismatch (version skew) happens
```
...
* Ensure there is not any orphen pods running.
* **Backups:**
...
```text
────────────────────────────────────────────────────────
KUBERNETES UPGRADE USING KUBEADM — SIMPLE EXPLANATION
────────────────────────────────────────────────────────
...
• Shows:
  ...
  - Components you must upgrade manually

--------------------------------------------------------

WHAT KUBEADM UPGRADES

kubeadm upgrades ONLY control plane components
that run as STATIC PODS:
...
You will see YAML files for each component.

--------------------------------------------------------

HOW THE UPGRADE WORKS INTERNALLY

Inside kube-apiserver.yaml you will see:
...
kubeadm changes the image version to:

image: registry.k8s.io/kube-apiserver:v1.33.x

kubeadm also update the manifests.

Then:

• kubelet detects file change
• Static pod restarts automatically
• New version runs

Upgrade = Change container image version

--------------------------------------------------------

WHAT KUBEADM DOES NOT UPGRADE

• kubelet (runs as system service)
• kubectl (client binary)

Why?
Because they are NOT pods.
They are installed as system binaries.

You must upgrade them manually:
...
COMPARE WITH APPLICATION UPGRADE
...
Kubernetes upgrade:
• kubeadm changes image version
• Static pods restart
• New control plane version runs
...
Upgrade = update container image version.

kubeadm automates this process.
...
kubectl get nodes
# Example: control-plane shows VERSION v1.32.8(kubelet version) until we upgrade kubelet below.
```
...
```bash
# Cordon: stop new pods from scheduling on this node(scheduler brinng node out of scheduling algo)
...
```text
────────────────────────────────────────────────────────
POD DISRUPTION BUDGET (PDB) — SIMPLE EXPLANATION
────────────────────────────────────────────────────────

Problem:

You have a critical deployment (red-deploy)
with 4 replicas.

You always want at least 3 pods running.

If you run:

kubectl drain worker-1 --ignore-daemonsets

All pods on that node will be evicted.

Without protection:
You may temporarily drop to 2 pods.
That is NOT acceptable for critical apps.

--------------------------------------------------------

Solution: PodDisruptionBudget (PDB)
...
At least 3 must always remain available.

--------------------------------------------------------

What Happens During Drain?

SCENARIO 1 — Cluster Has Capacity

• Drain starts
• Kubernetes evicts only 1 red pod (4 → 3)
• Replacement pod starts on another node
• Once new pod becomes Ready
• Next eviction allowed

Result:
Always 3 pods running.
Upgrade continues safely.

--------------------------------------------------------

SCENARIO 2 — No Capacity Available

• First pod evicted (4 → 3)
• Replacement cannot be scheduled
• Now only 3 running
• PDB blocks further eviction

Result:
Drain stops.
Node cannot be fully drained.
...
• PDB protects only voluntary disruptions
  (kubectl drain, rolling updates, etc.)

• If someone powers off the node,
  PDB does NOT help.

• Pod priority affects scheduling.
  PDB protects during eviction.
...
2. Ensure PDB is configured for critical apps
3. Prevent downtime during maintenance

Interview Answer:

"During upgrades, I drain nodes one by one and configure
PodDisruptionBudgets for critical workloads to ensure
