...
If annotation is not right then k8s not show any error but your ALB not configured properly.
...
```bash
┌──────────────────────────────────────────────────────────────────────────┐
│ MAIN IDEAS — ADVANCED INGRESS FEATURES & LIMITATIONS                    │
│                                                                          │
│ 1) CANARY / TRAFFIC SPLITTING                                           │
│ • Used during upgrades or feature testing                               │
...
│                                                                          │
│ 2) SSL / HTTPS REDIRECT                                                 │
│ • Automatically redirect HTTP → HTTPS                                   │
│ • Ensures secure access                                                 │
│ • Implemented via ingress annotations                                   │
│                                                                          │
│ 3) URL REWRITES                                                         │
│ • Modify incoming request paths                                         │
│ • Example:                                                              │
│   myapp.com/app1  →  myapp.com/app1/page                                 │
│                                                                          │
│ • Commonly used for backend compatibility                               │
│                                                                          │
│ 4) RATE LIMITING                                                        │
...
│                                                                          │
│ • Helps prevent abuse and overload                                      │
│                                                                          │
│ 5) HOW THESE FEATURES ARE IMPLEMENTED                                   │
│ • NOT native to Kubernetes Ingress spec                                 │
│ • Implemented using annotations                                         │
│ • Behavior depends on ingress controller                                │
│                                                                          │
│ 6) PORTABILITY PROBLEM WITH ANNOTATIONS                                 │
│ • Annotations are vendor-specific                                       │
│ • Not validated by Kubernetes API                                       │
│                                                                          │
│ • Example issue:                                                        │
│   • AWS ALB annotations ≠ NGINX annotations                              │
│   • Migrating controllers requires rewriting annotations               │
│                                                                          │
│ 7) KEY LIMITATION OF INGRESS                                            │
│ • Tight coupling to vendor/controller                                   │
│ • Poor portability across platforms                                     │
│                                                                          │
│ 8) WHY GATEWAY API IS INTRODUCED                                        │
│ • Next-generation replacement for Ingress                               │
│ • Standardizes advanced routing features                                │
│ • Supports multi-protocol traffic                                       │
│ • Reduces reliance on vendor-specific annotations                       │
│                                                                          │
│ SUMMARY                                                                 │
│ • Ingress supports powerful features via annotations                    │
│ • Canary, redirects, rewrites, rate limiting are common needs           │
│ • Annotations cause portability issues                                  │
│ • Gateway API solves these design limitations                            │
└──────────────────────────────────────────────────────────────────────────┘
```
