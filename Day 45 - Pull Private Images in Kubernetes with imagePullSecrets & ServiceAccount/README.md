...
Repositories is goiing to be private.
...
│                                                              │
│ • IRSA is an AWS EKS feature that allows Kubernetes Pods     │
│   to securely access AWS services using IAM Roles            │
│                                                              │
│ • Instead of giving permissions to the EC2 worker node,      │
│   permissions are given directly to a Kubernetes             │
│   ServiceAccount                                            │
│                                                              │
│ • Each Pod uses its own IAM Role → fine-grained access       │
│                                                              │
│ • Uses OIDC (OpenID Connect) between EKS and AWS IAM         │
│                                                              │
│ • No need to store AWS access keys inside Pods or images     │
│                                                              │
│ • More secure than node IAM roles                            │
│                                                              │
│ • Follows least-privilege security principle                 │
│                                                              │
│ HOW IT WORKS (FLOW)                                          │
│ • EKS cluster has an OIDC provider                           │
│ • IAM role trusts the OIDC provider                          │
│ • Role is mapped to a Kubernetes ServiceAccount              │
│ • Pod uses that ServiceAccount                               │
│ • AWS issues temporary credentials automatically             │
│                                                              │
│ EXAMPLE USE CASES                                            │
│ • Pod accessing S3 buckets                                   │
│ • Pod writing logs to CloudWatch                              │
│ • Pod accessing DynamoDB / SQS / SNS                          │
│                                                              │
│ WHY IRSA IS IMPORTANT                                        │
│ • Avoids credential leakage                                  │
│ • Multiple Pods can have different permissions               │
│ • Works well for production security                          │
│                                                              │
│ IRSA vs NODE IAM ROLE                                        │
│ • Node IAM Role → all Pods share permissions                  │
│ • IRSA → each Pod can have separate permissions               │
└──────────────────────────────────────────────────────────────┘
```
...
* Secretes specify at the pod spec used by all the containers of the pod.
